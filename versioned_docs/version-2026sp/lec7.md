...
specific docs such as the `jxc6` doc (which represents me!).

```typescript
const peopleCollectionRef = db.collection('people');
const jacDocRef = peopleCollectionRef.doc('jxc6');
```
...
```typescript
await peopleCollectionRef.set(jacDocRef, {
  year: '2026',
  age: 21,
});
```
...
.doc(‘jxc6’)
.update({ age: '21', first: 'jacqueline' });
```
